  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] pt-16">
      ...
      <div className="px-8 md:px-16 pt-8 pb-4">
        <Link
          href="/plataforma"
          className="text-gray-400 hover:text-[var(--gold)] text-sm transition-colors font-[var(--font-lato)] flex items-center gap-2"
        >
          ← Voltar à Plataforma
        </Link>
      </div>
      ...
      <div className="relative h-72 md:h-96 overflow-hidden mx-8 md:mx-16 rounded-2xl mb-10">
        ...
        <div className="relative z-10 h-full flex flex-col justify-end p-8 max-w-2xl">
          <h1 className="text-3xl md:text-5xl font-[var(--font-playfair)] font-black text-white mb-2">
            {course.title}
          </h1>
          <p className="text-[var(--gold-pale)] font-[var(--font-playfair)] italic mb-4">
            {course.subtitle}
          </p>
      ...
      <div className="px-8 md:px-16 mb-10">
        <div className="flex flex-wrap gap-6 text-sm text-gray-300 font-[var(--font-lato)] mb-4">
          ...
          <span>{course.totalHours}h de conteúdo</span>
        </div>
        <p className="text-gray-400 max-w-3xl leading-relaxed font-[var(--font-lato)]">
          {course.description}
        </p>
      </div>
        ...
        {course.modules.map((mod, modIdx) => (
          <div key={mod.id} className="rounded-xl border border-[var(--border)] overflow-hidden">
            ...
            <div className="bg-[var(--card)] px-6 py-4 flex items-center gap-4">
              ...
              <div>
                <h3 className="text-white font-[var(--font-playfair)] font-bold">
                  {mod.title}
                </h3>
              ...
              {mod.lessons.map((lesson, lessonIdx) => (
                ...
                >
                  ...
                  <div className="w-8 h-8 rounded-full border border-[var(--border)] flex items-center justify-center group-hover:border-[var(--gold)] group-hover:bg-[var(--gold)]/10 transition-all flex-shrink-0">
                    <span className="text-gray-500 group-hover:text-[var(--gold)] text-xs ml-0.5 transition-colors">
                      ▶
                    </span>
                  </div>
                  {/* Info */}
                  ...
                  {/* Duration */}
                  <span className="text-gray-500 text-xs font-[var(--font-lato)] flex-shrink-0">
                    {lesson.duration}
                  </span>
                </Link>
              ))}
